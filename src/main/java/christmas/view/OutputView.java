...
public class OutputView {
    private static final String START_MESSAGE = String.format("안녕하세요! 우테코 식당 %d월 이벤트 플래너입니다.", EVENT_MONTH);
    private static final String RESULT_HEADLINE_MESSAGE = String.format("12월 %d일에 우테코 식당에서 받을 이벤트 혜택 미리 보기!", EVENT_MONTH);
    ...
    private static final String BENEFITED_AMOUNT = "<할인 후 예상 결제 금액>\n%d원";
    ...
    private String getGiftString(boolean canGetGift) {
        if (canGetGift) {
            return String.format(MENU_LIST, CHAMPAGNE, 1);
        }
        return NO_BENEFIT;
    }
}
